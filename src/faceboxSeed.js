    ...
    const requests = re.map(x => rp({
        ...
        .catch(err => {
            if (err.statusCode === 404) {
                console.error(chalk.red(`The route you provided for Facebox is not reachable: ${facebox}`));
                console.error(chalk.red('Make sure you specified the correct URL, and that Facebox is not in read only mode'));
                process.exit();
            }
            console.error(chalk.red(`Issue with ${err.options.body.url}: ${err.error.error || err.message}`));
        }));

    // Report Totals
    console.log(chalk.cyan(`Found a total of ${requests.length} Images in Album ${album}`));
...
const handleSeed = async (name, albumUrl, faceboxUrl, opts) => {
    ...
    console.log(chalk.green(`Processed a total of ${results.totalSuccessful} Images succesfully`));
    ...
    if (opts.state) {
        ...
        try {
            ...
            console.log(chalk.green(`Downloaded the State File to ${process.env.PWD}/${filename}`));
        } catch (err) {
            console.error(chalk.red(`An error occured trying to download the State File: ${err.message}`));
        }
    }
};
